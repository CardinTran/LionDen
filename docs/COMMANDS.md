...
- `/profile`: show the caller's level, XP, coins, progress, and favorite lion with compact bond flavor when set
...
LionDen also has public `~` text commands for red envelope claims and lion creature gameplay, including `~favorite <lion>`, `~showcase [lion]`, `~bond [lion]`, `~feed [lion]`, and `~groom [lion]` for social lion identity and cosmetic care. See [LION_TEXT_COMMANDS.md](LION_TEXT_COMMANDS.md) for the text-command surface, router entry points, handler files, and maintenance-mode behavior.
